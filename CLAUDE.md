...
> **gitignored** — 로컬 전용. JSON 작업 시 개념·오개념 판단의 최우선 기준.

### ⚠️ 폴더 구분 — 반드시 숙지

| 폴더 | 용도 | Claude 접근 |
|------|------|------------|
| `references/` | PDF 원본 보관소 (강사 답안, 원전 교재 등) | **✅ Read 툴로 직접 읽기 가능** — 그림·표 포함. 단 Grep 불가 |
| `refs/` | MD 변환본 참조 폴더 | **✅ Grep + Read 모두 가능** — 키워드 검색 최적 |

**용도 구분**:
- 키워드 빠른 검색 → `refs/` MD 우선
- 특정 페이지·그림·표 확인 → `references/` PDF를 Read 툴로 직접 열기
- 강사 답안 확인 → `refs/` MD 변환본 우선, 없으면 `references/` PDF 직접 Read

| 파일 | 용도 | 내용 | 분량 |
|------|------|------|------|
| `refs/subnote_all.md` | 영어교육론 개념 검증 | 영어교육론 서브노트 전체 (Ch.1–13, kimme 기반) | ~4200줄 |
| `refs/keywords.md` | 영어교육론 키워드 맥락 | 연도별 기출 키워드 정리 + 출처(PLLT 등) | ~1600줄 |
| `refs/mapping.md` | 영어교육론 이론 계보 | 교수법 개념 맵핑 (루이스 기반, Behaviorism→Constructivism 계보) | ~1800줄 |
| `refs/역대기출답안.md` | **모범답안 1차 근거** | 2014–2023 강사(권영주 등)+합격자 기출 모범답안 전체 | ~506KB |
| `refs/영교론 기출 키워드 정리.md` | 영어교육론 키워드·출처 보완 | 연도별 기출 키워드 + PLLT 등 원전 출처 대조 정리 | ~120KB |
| `refs/윤영어학.md` | **영어학 개념 검증** | "What English Language Teachers Should Know" 윤 영어학 참조서 | ~608KB |
| `refs/TG.md` | **영어학 통사론 원전** | Radford, *Transformational Grammar* (1988/2009) 전문 | ~1.4MB |
| `refs/모범답안_2014-2023.md` | **강사+합격자 답안** | 역대 기출 강사 및 합격자 모범답안 모음 (2014–2023) | ~526KB |
| `refs/루이스기출문제.md` | 일반영어 기출 분석 | 루이스 기출문제 텍스트 변환본 | ~643KB |
| `refs/메가쌤_기출분석.md` | 기출 분석 | 메가쌤 전공영어 기출 분석 텍스트본 | ~92KB |
| `refs/밍우_영어학분석.md` | 영어학 분석 | 밍우 영어학 분석 텍스트 변환본 | ~37KB |
| `refs/밍우_일영문학분석집.md` | 일반영어(문학) 분석 | 밍우 일영문학 분석집 텍스트 변환본 | ~258KB |
| `refs/2024 유희태 답안.md` | 강사 답안 (2024) | 유희태팀 2024 기출 모범답안 | ~13KB |
| `refs/2024 조셉신 답안.md` | 강사 답안 (2024) | 조셉신 2024 기출 모범답안 | ~7KB |
| `refs/2025 유희태 답안.md` | 강사 답안 (2025) | 유희태팀 2025 기출 모범답안 | ~15KB |
| `refs/2026 강사별 답안.md` | 강사 답안 (2026) | 강사별 2026 기출 모범답안 | ~29KB |

### 언제 참조하는가

**영어교육론 작업 시:**
- **모범답안·채점 포인트** 확인: `refs/역대기출답안.md` → 연도/번호로 검색, 강사·합격자 답 대조
- **개념 정의** 작성 시: `refs/subnote_all.md` → 이론가·연도·정의 문구 일치 여부 확인
- **오개념 의심** 시: `refs/subnote_all.md` 해당 챕터 검색 → 정확한 정의 대조
- **키워드 맥락** 확인: `refs/keywords.md` 또는 `refs/영교론 기출 키워드 정리.md` → 연도별 출제 패턴
- **이론 계보·관계** 파악: `refs/mapping.md` → 상위/하위 개념 위치 확인

**영어학 작업 시:**
- **통사론** (구조·이동·변형): `refs/TG.md` (Radford) → 트리 구조·용어 정의 검증
- **일반 영어학 개념**: `refs/윤영어학.md` → 음운론·형태론·통사론·의미론 개념 확인

### refs/ 활용 원칙

1. `refs/역대기출답안.md`는 **답안 작성의 1차 근거** — 해당 연도 문항 답이 있으면 반드시 대조 후 작성.
2. `subnote_all.md` 서술과 다른 정의는 **반드시 재확인** — Claude 독자 판단으로 덮어쓰기 금지.
3. 특히 Communicative Competence 계보, WCF 유형, Interaction Hypothesis, Noticing Hypothesis 등 **자주 오개념이 발생하는 항목**은 refs 대조 필수.
4. 합격자 서술이 원전 교재와 미세하게 다를 경우 → **합격자 서술 우선** (시험 채점 기준에 더 근접).
5. `refs/` 파일은 직접 인용 금지 — 내용 확인 후 JSON에는 재작성하여 기재.
6. 영어학 JSON 작성 시 통사론 문항: `refs/TG.md`에서 관련 규칙·트리 형태 확인 후 `derivation.steps` 작성.
