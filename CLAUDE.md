...
| 폴더 | 용도 | Claude 접근 |
|------|------|------------|
| `references/` | PDF 원본 보관소 (강사 자료, 원전 교재 등) | **❌ 읽지 않음** — PDF는 Grep 불가 |
| `refs/` | MD 변환본 참조 폴더 | **✅ 여기만 참조** — Grep/Read 가능 |

**워크플로우**: `references/` PDF → 텍스트 변환 → `refs/`에 MD로 저장 → Claude가 참조  
새 자료를 Claude가 쓰게 하려면 반드시 **`refs/`에 MD로 넣어야** 한다.
