...
**일반영어·문학 파일 작성 시 체크리스트 (하이라이트 실수 방지):**
- [ ] `derivation.key_terms` 는 `[]` (일반영어·문학 공통)
- [ ] **모든 step.key_terms 에 최소 1개 이상** 채워져 있는가 — 5개 step 중 빈 step이 있으면 해당 step은 em 하이라이트 없음
- [ ] step 3(후보 소거)에는 정답 단어가 반드시 포함 — 예: `['savages']`, `['nothing']`
- [ ] step 4(확정 근거)에는 `[content]/[form]/[topic]/[support]/[conclusion]` 태그 뒤 인용구의 핵심어 포함
- [ ] 모든 key_term 이 해당 step.explanation 텍스트에 **문자열 그대로** (대소문자 무시) 등장하는지 검증
- [ ] `question_type`: 빈칸 단답은 `short_answer`, 서술형(essay_content 있음)은 `essay` — 혼용 금지
- [ ] `blank_answers[].answer` 가 있으면 자동으로 정답 하이라이트 대상이 되므로 정확한 지문 내 형태로 기재

**자동 검증 스니펫 (파일 저장 전 1회 실행):**
```bash
python -c "
import json,sys
f=sys.argv[1]
d=json.load(open(f,encoding='utf-8'))
for s in d['derivation']['steps']:
    t=s['explanation'].lower()
    miss=[k for k in s.get('key_terms',[]) if k.lower() not in t]
    empty = not s.get('key_terms')
    if miss: print(f'step {s[\"step\"]} missing: {miss}')
    if empty: print(f'step {s[\"step\"]} EMPTY key_terms')
" data/파일명.json
```
결과에 `EMPTY` 또는 `missing` 출력이 남아 있으면 커밋 금지.
