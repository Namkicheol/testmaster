...
> **신뢰도 최우선**: 모든 답안은 역대 강사(이동걸, 최진호, 유희태팀, 권영주, 최시원 등) 및 합격자 답안을 1차 근거로 삼는다. Claude가 독자적으로 창작하지 않는다.

### 답안 신뢰도 원칙 (최우선)

- **단답형**: 역대 강사 공통 답 → 모범답안. 이견 없으면 그대로.
- **서술형**: 역대 강사 + 합격자 답안을 분석 → 공통 채점 포인트 추출 → 재작성.
  - Claude 단독 판단으로 작성 금지. 반드시 강사/합격자 답 바탕.
  - 강사 간 이견 → 다수 의견 채택. 합격자 답과 교차 검증.
- **variants**: 강사 답(A) / 합격자 답(B) 스타일을 각각 반영해 재작성. 실명 금지.
...
```
model_answer  → 강사·합격자 공통 채점 포인트 기반, 합격자 스타일로 재작성한 대표 답안
variants[A]   → 강사 답안 스타일 반영 재작성 (문장 구조 변형)
variants[B]   → 합격자 답안 스타일 반영 재작성 (증거 제시 방식 변형)
```
- **반드시 강사/합격자 답 바탕** — Claude 독자 창작 금지.
